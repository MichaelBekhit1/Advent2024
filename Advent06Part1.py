...
class Guard:
    ...
    def proceed(self):
        ...
        while 0 < self.target_coords[0] < cols and 0 < self.target_coords[1] < rows:
            print("x")
            # runs 8 times
            ...
            if self.grid[self.target_coords[0]][self.target_coords[1]] == "#":
                ...
                new_y = self.target_coords[0]+(directions[directions_index][0])
                new_x = self.target_coords[1]+(directions[directions_index][1])
                #print(new_y)
                self.target_coords=(new_y, new_x)#((self.target_coords[0] + self.directions[directions_index][0]),(self.target_coords[1] + self.directions[directions_index][1]))
                print(new_x, new_y, self.target_coords)
                ...
                new_y = self.target_coords[0]+(directions[directions_index][0])
                new_x = self.target_coords[1]+(directions[directions_index][1])
                self.target_coords=(new_y, new_x)
...
for col, row in enumerate(guard.proceed()):
    print(col,row)
# print(1 + directions[directions_index][0])